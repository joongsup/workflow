...
We can describe what the source R script file does here.
...
- Note any objects in the src_r_script.R that I want to be displayed in the rendered document had to be saved to a variable and printed manually. Check out [R/src_r_script.R](R/src_r_script.R) file. Without such manual print command, the plot wouldn't display in rendered Rmd document.  
...
- Interestingly, the table gets printed in interactive Rmd session, but not the plot. Instead it prints the following warning:
    - "UserWarning: Matplotlib is currently using agg, which is a non-GUI backend, so cannot show the figure."
    - I didn't know what [backend](https://matplotlib.org/faq/usage_faq.html#what-is-interactive-mode) even was before, but it seemed quite important in making sure plots are printed on the final rendered document.

## Working R and Python together
...
1. Start three Vim/REPL paired sessions
    - Session Rmd: for home (Rmd) document editing and interactive R computing
    - Session R: for R script editing and interactive R computing
    - Sesion Py: for Py script editing and interactive Python computing

2. Start documenting project progress
    - Provide context and describe data, etc. 

3. Iteratively work on codes (R/Python), generate plots, and document findings 
    - Make sure codes run successfully by checking outputs from each corresponding session (e.g., check R codes work in session R, and Python codes work in session Py) 
